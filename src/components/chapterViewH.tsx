            ...
            <div className='flex flex-row w-full gap-2 h-52 bg-[--secondary]'>
                <a href={"/c/" + chapter.$id} className='flex flex-row w-full h-full gap-2'>
                    ...
                    <div className='flex flex-col w-full h-full grow'>
                        <h2 className='text-xl font-bold text-[--primaryText] mb-2'>{chapter.title}</h2>
                        <h3 className='mb-1 text-sm font-medium text-[--primaryText]'>{chapter.Titles && (chapter.Titles.name + " ")}#{formatedNumber}</h3>
                        <h3 className='mb-2 text-sm font-medium text-[--primaryText]'>By: <a href={"/u/" + chapter.Author.$id} className='underline'>{chapter.Author.name}</a></h3>
                        <p className='mt-auto text-sm font-medium text-[--secondaryText] mb-2'>Posted {howLongAgo}</p>
                    </div>
                </a>
            ...
            <Drawer.Portal>
                <Drawer.Overlay className='fixed inset-0 z-40 bg-black/50' />
                    <Drawer.Content className='bg-[--background] border-2 z-50 border-[--primary] flex flex-col items-center rounded-t-[10px] max-h-[80%] fixed bottom-0 left-0 right-0'>
                        <div className={`max-w-[1000px] w-full flex flex-col p-4 rounded-t-[10px] overflow-y-auto`}>
