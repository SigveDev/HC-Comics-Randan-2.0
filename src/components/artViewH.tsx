  ...
  const handleShare = () => {
    if (art) {
      ...
      if (navigator.share) {
        ...
        const plainUrl =
          window.location.href.replace(newUrl, "") + "a/" + art.$id;
        navigator
          .share({
            title: art.title,
  ...
  return (
    ...
    >
      <div className="w-full h-fit">
        <img
          src={artImagePreview?.href}
          alt="Art"
          className="w-full aspect-[2/3] cursor-pointer"
          onClick={handleModal}
        />

        {modal && (
          <div
            className="fixed top-0 bottom-0 left-0 right-0 flex items-center justify-center w-full h-full bg-black/50"
            onClick={handleModal}
          ></div>
        )}
        {modal && (
          <div className="fixed top-0 bottom-0 left-0 right-0 z-10 flex items-center justify-center w-full h-full pointer-events-none">
            <div className="flex flex-col items-center justify-start gap-2 overflow-y-auto pointer-events-auto lg:overflow-visible xl:overflow-visible xl:flex-row lg:flex-row w-fit h-4/5">
              <img
                src={artImage?.href}
                alt="Art"
                className="xl:h-full lg:h-full h-fit md:w-2/3 sm:w-4/5 w-4/5 aspect-[2/3]"
              />
