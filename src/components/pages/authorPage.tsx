...
import { Title, Author, Art, LikesRequest, Following, FollowingRequest } from "../../assets/types";
import { getAuthorPFP, getAuthorById, checkUserData, getLiked, getFollowing, followAuthorToggle } from "../../lib/Appwrite";
        ...
        <div className='flex flex-col items-center justify-center w-full gap-8 mt-8 mb-8 h-fit'>
            <div className="w-full h-fit">
                <div className="flex flex-row w-full h-24 gap-4 px-4 mt-8 xl:px-28 lg:px-28">
                    <img className="object-cover h-full rounded-full aspect-square" src={authorPFP} alt="Author Image" />
                    <div className="flex flex-col w-full h-full">
                        <div className="flex flex-row justify-between">
                            <h1 className="text-2xl font-bold text-[--primaryText]">{author?.name}</h1>
                            {loggedIn && (
                                followStatus ?
                                ...
                                :
                                <button className="text-1xl font-semibold text-[--primaryText] flex flex-row gap-1 items-center" onClick={toggleFollowing}>Follow Author <Plus /></button>
                            )}
                        </div>
                ...
                <div className="flex justify-center w-full h-fit">
                    <div className="flex flex-col w-full gap-4 mx-4 xl:w-1/2 lg:w-1/2">
                        <h2 className="text-2xl font-bold text-[--primaryText] w-full flex justify-center">Creations:</h2>
            ...
            {tabb === "chapters" ? <div className='grid w-full h-full grid-cols-1 gap-12 px-4 py-8 xl:px-12 lg:px-12 xl:grid-cols-3 lg:grid-cols-3'>
