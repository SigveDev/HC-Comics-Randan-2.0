    ...
    useEffect(() => {
        const checkAmountOfPages = async () => {
            ...
            if (currentPage !== null) {
                ...
                if (maxPage && (chapterStorage.length - 1) >= (maxPage * 0.55) && chapterState === false) {
                    await giveChapterView(chapterID);
                    localStorage.removeItem("currentChapter");
                    await addChapterToHistory(chapterID);
                } else if (maxPage && (chapterStorage.length - 1) >= (maxPage * 0.75) && chapterState === true) {
                    await giveChapterView(chapterID);
                    localStorage.removeItem("currentChapter");
                } else if (maxPage && (chapterStorage.length - 1) >= (maxPage * 0.55) && chapterState === undefined) {
                    await giveChapterView(chapterID);
                    localStorage.removeItem("currentChapter");
                }
            }
