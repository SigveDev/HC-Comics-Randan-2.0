...
const ChaptersList = ({ chapters }: { chapters: Chapter[] }) => {
    ...
    const handleSort = () => {
        if (isASC) {
            const sorted = [...chapters].sort((a, b) => b.title.localeCompare(a.title));
            setSortedChapters(sorted);
        } else {
            const sorted = [...chapters].sort((a, b) => a.title.localeCompare(b.title));
            setSortedChapters(sorted);
        }
        setIsASC(!isASC);
    }
    ...
    return (
        <div className='flex flex-col w-full gap-2 h-fit'>
            <p className='flex items-center justify-start h-7 pl-2 font-semibold text-[--primaryText] bg-gradient-to-r from-[--primary] via-[--thirdly] via-55% to-transparent'>Chapters <button onClick={handleSort}>{isASC ? <ChevronDown /> : <ChevronUp />}</button></p>
            {sortedChapters.map((chapter: Chapter) => {
                ...
                return (
                    <ChapterViewH chapter={chapter} likedStatus={likedStatus} loggedIn={loggedIn} userId={userId as string} key={chapter.$id} />
                );
            })}
        </div>
    );
