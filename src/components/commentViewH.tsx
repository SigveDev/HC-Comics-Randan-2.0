...
const CommentViewH = ({
  ...
  useEffect(() => {
    const fetchComments = async () => {
      if (chapterOrNot) {
        const comments: CommentsRequest = (await getCommentsByChapterId(
          id
        )) as CommentsRequest;
        setCommentList(comments.documents);
      } else {
        const comments: CommentsRequest = (await getCommentsByArtId(
          id
        )) as CommentsRequest;
        setCommentList(comments.documents);
      }
  ...
  return (
    <div className="flex flex-col w-full gap-2 h-fit">
      ...
      {loggedIn && (
        <div className="flex flex-row w-full gap-4 mb-4 h-fit">
          <img src={userPFP} alt="UserPfp" className="rounded-full w-14 h-14" />
          <div className="grow h-fit">
            <textarea
              className="w-full p-2 bg-[--background] border-b-2 border-[--primary] caret-[--primaryText] text-[--primaryText] outline-none resize-none overflow-hidden rounded-none"
            ...
            <div className="flex flex-row items-center justify-end w-full">
              <button
                type="button"
                className="flex items-center justify-center w-8 h-8 font-semibold text-[--primaryText] rounded"
                onClick={handlePost}
              >
                <SendHorizonal />
              </button>
      ...
      <div className="flex flex-col w-full gap-4 h-fit">
        {commentList.length > 0 &&
          commentList.map((commentData: Comments) => {
            return (
              <Comment
                commentData={commentData}
                publicUserId={publicUserId as string}
                key={commentData.$id}
              />
            );
          })}
